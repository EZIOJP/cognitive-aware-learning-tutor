...
- memory: long-term memory — remembered_facts (goals, deadlines, preferences, struggles from past
  chats) and recent_days (one summary line per day of study/sleep/vocab metrics). Use these to
  stay consistent across sessions: reference their goals, notice trends across recent_days,
  and never ask about things already in remembered_facts.
...
{_context_block(hub_payload)}"""
    ...
    raw = ollama_generate(prompt, system_prompt=COACH_SYSTEM, timeout=90.0, task="coach")
    if not raw:
