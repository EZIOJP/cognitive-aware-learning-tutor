...
def _run_texteller(prepared: Image.Image) -> str:
    """Legacy helper — prefer ocr_engine.recognize_crop on raw crops."""
    from backend.math.texteller_onnx import recognize_image

    return recognize_image(prepared).strip()


def _run_ocr_crop(crop: Image.Image) -> tuple[str, float, str]:
    """Dual-engine OCR + SymPy repair on one band crop."""
    from backend.math.ocr_engine import recognize_crop

    out = recognize_crop(crop)
    return out.latex, out.confidence, out.source
...
def _per_cell_ocr(img: Image.Image, stroke_metrics_json: str | None, pad: int = 12) -> str:
    ...
    for x0, y0, x1, y1 in boxes:
        ...
        if not image_has_ink(crop, min_ink_pixels=20):
            continue
        try:
            piece, _conf, _src = _run_ocr_crop(crop)
        except Exception:
            continue
...
def recognize_multiline(
    ...
    *,
    ...
    if not texteller_available() and not unimernet_available():
        return OcrResult(
            latex="",
            incomplete_step=True,
            confidence=0.0,
    ...
    if len(bands) >= 2:
        ...
        for band in bands:
            ...
            latex = ""
            conf = 0.0
            band_src = band.source
            try:
                latex, conf, band_src = _run_ocr_crop(crop)
            except Exception:
                latex = ""
            if latex and _ocr_looks_hallucinated(latex):
                ...
                conf = 0.0
            bbox = band.as_bbox()
            latex, conf, src, ens_review = maybe_disambiguate_latex(
                latex,
                confidence=conf,
            ...
            line_results.append(
                OcrLineResult(
                    ...
                    source=src
                    if src in ("stroke_symbol", "ensemble_agree", "nim_agree", "unimernet", "texteller")
                    else band_src,
                )
            )
            if ens_review:
    ...
    ocr_src = "texteller"
    try:
        latex, conf_whole, ocr_src = _run_ocr_crop(cropped)
    except Exception:
        latex = ""
        conf_whole = 0.0
    if latex and _ocr_looks_hallucinated(latex):
        ...
        tier = ocr_src if len(bands) < 2 else f"{ocr_src}_fallback"
    ...
    if latex and conf_whole > 0:
        result.confidence = max(result.confidence, conf_whole)
    result.tier = tier if latex else "tier3_empty"
    bbox = {"x": 0, "y": 0, "w": img.size[0], "h": img.size[1]}
    if bands:
        bbox = bands[0].as_bbox()
    latex2, conf2, src, ens_review = maybe_disambiguate_latex(
        result.latex,
    ...
    if src in ("stroke_symbol", "ensemble_agree", "nim_agree"):
        result.latex = latex2
...
def recognize_canvas(
    ...
    *,
    ...
    if paths_json:
        paths_json = normalize_paths_json_stroke_order(paths_json) or paths_json
    ...
    if multiline:
        ...
        tier = "none"
        try:
            latex, _c, tier = _run_ocr_crop(cropped)
        except Exception:
            latex = ""
