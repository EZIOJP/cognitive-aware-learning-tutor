    ...
    *,
    ...
    if total < threshold:
        return RetrainExportResult(
            ...
            message=f"Need at least {threshold} samples; have {total}.",
            total_samples=total,
            exported=0,
            skipped=total,
    ...
    for row in data:
        ...
        jsonl_rows.append({"image": dest_name, "formula": latex, "sample_id": sample_id})
    ...
    if exported < threshold:
        return RetrainExportResult(
            ...
            message=(
                f"Only {exported} rows had PNG + label (need {threshold}). "
    ...
    with open(FORMULAS_JSONL, "w", encoding="utf-8") as f:
    ...
    return RetrainExportResult(
        status="exported",
        message=f"Exported {exported} samples to {_display_path(TRAIN_DIR)}.",
        total_samples=total,
        exported=exported,
        skipped=total - exported,
        skip_reasons=skip_reasons,
