...
class GenerateNotesRequest(BaseModel):
    ...
    asr_backend: str = Field(default="recasepunc", max_length=32)
    note_style: str = Field(default="bullets", max_length=16)
    coherence_mode: str = Field(default="compact", max_length=16)
    semantic_threshold: float = Field(default=0.45, ge=0.0, le=1.0)
    semantic_threshold_mode: str = Field(default="fixed", max_length=16)
    semantic_chunk_percentile: float = Field(default=95.0, ge=50.0, le=99.9)
...
def get_llm_settings(
    ...
    from backend.core.llm_gateway import get_gateway_config
    from backend.corpus.retrieve import corpus_available
...
def generate_notes(
    ...
    try:
        ...
        path, content, mode, rag_result = generate_notes_unified(
            ...
            enrich_visuals=body.enrich_visuals,
        )
    except RuntimeError as exc:
        log.exception("POST /notes/generate runtime error for %s", body.transcript_file)
        raise HTTPException(status_code=503, detail=str(exc)) from exc
    except ValueError as exc:
        log.exception("POST /notes/generate validation error for %s", body.transcript_file)
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    ...
    if mode.startswith("legacy"):
        ...
        corpus_handoff = rag_result.get("corpus_handoff")
    from backend.paths import NOTES_DIR
