...
def _embed_rules(rules: str) -> str:
    """Escape mermaid example braces so str.format only fills chunk/reference/body."""
    return _escape_format_braces(rules)


REFINE_PROMPT = f"""Polish these lecture notes into one cohesive markdown document.
- Fix duplicate headings and merge overlapping bullets
- Keep mermaid and code blocks intact; apply strict Mermaid rules when fixing diagrams
...
def summarize_chunk(
    ...
    *,
    ...
    body = CHUNK_PROMPT.format(chunk=chunk_text, reference=ref)
    if aggressive:
        prompt = (
...
def generate_notes_from_text(
    ...
    **kwargs: object,
    ...
    body = finalize_note_markdown(body)
    progress(
