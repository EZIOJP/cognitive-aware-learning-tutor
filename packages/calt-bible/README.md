...
- **Notify:** hourly verse from the current chapter, 08:00–21:00 (toggle in Settings)
...
3. Confirm version **1.0.5** on the watch
...
Phone: Developer Mode → + → Scan the QR. **Uninstall** the old CALT Bible first, then install so the watch gets **1.0.5**.
