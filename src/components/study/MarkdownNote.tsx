...
export function MarkdownNote({ content, sectionEdit, previewMode = "full" }: MarkdownNoteProps) {
  ...
  return (
    <div className="lecture-notes-markdown space-y-3 leading-relaxed">
      <ReactMarkdown
        remarkPlugins={[remarkGfm]}
        components={{
          h1: ({ children }) => (
            <h1 className="font-bold mt-4 mb-3 text-foreground border-b pb-2">{children}</h1>
          ),
          h2: ({ children }) => (
            <h2 className="font-semibold mt-6 mb-2 text-foreground">{children}</h2>
          ),
          h3: ({ children }) => (
            <h3 className="font-medium mt-4 mb-2 text-foreground/95">{children}</h3>
          ),
          ...
          p: ({ node, children }) => {
            ...
            return <p className="text-foreground/90 my-2">{children}</p>;
          },
          blockquote: ({ children }) => (
