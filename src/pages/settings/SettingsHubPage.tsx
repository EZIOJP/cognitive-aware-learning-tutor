import { useState } from "react";
import { Link } from "react-router";
import { Settings2, Palette, Puzzle, Sun, Moon, Lightbulb } from "lucide-react";
import { ThemeToggle } from "../../components/theme/ThemeToggle";
import { Card } from "../../app/components/ui/card";
import { Button } from "../../app/components/ui/button";
import { SettingsPageScroll } from "./SettingsPageScroll";
import {
  CaltAndroidDownloadCard,
  CaltAndroidDownloadCardHeader,
} from "./CaltAndroidDownloadCard";
import { AiSettingsSummaryCard } from "./AiSettingsSummaryCard";
import { NineRouterCard } from "./NineRouterCard";

const VARIANT_KEY = "themeToggleVariant";

type ToggleVariant = "animated" | "compact";

function readVariant(): ToggleVariant {
  const v = localStorage.getItem(VARIANT_KEY);
  return v === "compact" ? "compact" : "animated";
}

export default function SettingsHubPage() {
  const [variant, setVariantState] = useState<ToggleVariant>(readVariant);

  const setVariant = (v: ToggleVariant) => {
    localStorage.setItem(VARIANT_KEY, v);
    setVariantState(v);
    window.dispatchEvent(new Event("theme-toggle-variant"));
  };

  return (
    <SettingsPageScroll className="p-4 md:p-6">
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-6">
        <div className="gloss-panel rounded-3xl border border-border/50 p-5">
          <div className="flex items-start gap-3">
            <div className="grid h-10 w-10 place-items-center rounded-2xl border border-primary/20 bg-primary/10">
              <Settings2 className="h-5 w-5 text-primary" />
            </div>
            <div>
              <h1 className="text-2xl font-semibold">Settings</h1>
              <p className="text-sm text-muted-foreground">
                Personalize appearance, AI routing, mobile setup, and enabled modules.
              </p>
            </div>
          </div>
        </div>

        <div className="grid gap-6 xl:grid-cols-[minmax(0,1.1fr)_minmax(20rem,0.9fr)]">
          <div className="space-y-6">
            <Card className="gloss-panel p-5 space-y-5">
              <div className="flex items-center gap-2">
                <Palette className="w-5 h-5 text-primary" />
                <h2 className="font-medium">Appearance</h2>
              </div>
              <p className="text-sm text-muted-foreground">
                Tap the control to switch light / dark. Icons animate between sun and moon.
              </p>
              <div className="flex flex-wrap items-center gap-4 rounded-2xl border border-border/50 bg-background/35 p-4">
                <ThemeToggle size="md" variant={variant} />
                <div>
                  <p className="text-sm font-medium">Live preview</p>
                  <p className="text-xs text-muted-foreground">Matches the top bar after refresh.</p>
                </div>
              </div>
              <div>
                <p className="text-sm font-medium mb-2">Toggle style</p>
                <div className="flex flex-wrap gap-2">
                  <Button
                    type="button"
                    variant={variant === "animated" ? "default" : "outline"}
                    size="sm"
                    onClick={() => setVariant("animated")}
                  >
                    <Sun className="w-4 h-4 mr-1" />
                    Animated sky
                  </Button>
                  <Button
                    type="button"
                    variant={variant === "compact" ? "default" : "outline"}
                    size="sm"
                    onClick={() => setVariant("compact")}
                  >
                    <Moon className="w-4 h-4 mr-1" />
                    Compact slider
                  </Button>
                </div>
              </div>
              <Link
                to="/settings/theme"
                className="text-sm text-primary hover:underline inline-block"
              >
                Advanced theme (accent, radius, intensity) →
              </Link>
            </Card>

            <AiSettingsSummaryCard />

            <NineRouterCard />
          </div>

          <div className="space-y-6">
            <Card className="gloss-panel p-5">
              <CaltAndroidDownloadCardHeader />
              <CaltAndroidDownloadCard />
            </Card>

            <Card className="gloss-panel p-5">
              <div className="flex items-center gap-2 mb-2">
                <Lightbulb className="w-5 h-5 text-amber-400" />
                <h2 className="font-medium">ESP32 LED</h2>
              </div>
              <p className="text-sm text-muted-foreground mb-3">
                Play with the board RGB over USB — no mic needed.
              </p>
              <Link
                to="/eeg"
                className="inline-flex items-center rounded-xl bg-amber-500/20 px-3 py-2 text-sm font-medium text-amber-200 hover:bg-amber-500/30"
              >
                Open LED play →
              </Link>
            </Card>

            <Card className="gloss-panel p-5">
              <div className="flex items-center gap-2 mb-2">
                <Puzzle className="w-5 h-5 text-primary" />
                <h2 className="font-medium">Plugins</h2>
              </div>
              <p className="text-sm text-muted-foreground mb-3">
                Enable or disable study modules.
              </p>
              <div className="flex flex-col gap-1">
                <Link to="/settings/plugins" className="text-sm text-primary hover:underline">
                  Manage plugins →
                </Link>
                <Link to="/settings/features" className="text-sm text-primary hover:underline">
                  Feature Studio (custom metrics) →
                </Link>
              </div>
            </Card>
          </div>
        </div>
      </div>
    </SettingsPageScroll>
  );
}
