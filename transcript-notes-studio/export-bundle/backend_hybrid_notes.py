...
from backend.transcripts.cleanup import clean_transcript, count_code_blocks, count_mermaid_blocks, looks_like_live_captions
...
def _generate_hybrid_grounded_notes_impl(
    *,
    ...
    if not ollama_available(llm):
        raise RuntimeError("Local LLM required for hybrid grounded notes.")
    ...
    chunks = _select_chunks(cleaned, use_semantic_grouping=True, fast_mode=False)
    ...
    if refine_second_pass:
        ...
        if refined and refined.strip():
            body = polish_chunk_text_only(refined)
