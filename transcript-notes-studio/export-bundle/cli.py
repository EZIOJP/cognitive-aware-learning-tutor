...
def _cmd_capture(args: argparse.Namespace) -> int:
    ...
    idle_seconds = getattr(args, "idle", None)
    print("Capturing Live Captions — Ctrl+C to stop…", file=sys.stderr)
    try:
        scraper.run(max_seconds=max_seconds, idle_seconds=idle_seconds)
...
def _cmd_lecture_auto(args: argparse.Namespace) -> int:
    from transcript_studio.config import save_config
    from transcript_studio.lecture_auto import run_lecture_auto

    cfg = load_config()
    if args.idle is not None:
        cfg.lecture_auto_idle_sec = max(60.0, float(args.idle))
    if args.max_duration is not None:
        cfg.lecture_auto_max_sec = max(0.0, float(args.max_duration))
    if args.no_rag:
        cfg.lecture_auto_use_rag = False
    if args.fast:
        cfg.lecture_auto_fast_mode = True
    save_config(cfg)

    def on_phase(phase: str, message: str) -> None:
        print(f"[{phase}] {message}", file=sys.stderr)

    result = run_lecture_auto(cfg, on_phase=on_phase, on_progress=on_phase)
    if result.success:
        print(f"Done — notes: {result.note_path}")
        return 0
...
def _cmd_generate(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        path, _body, mode = generate_notes_from_file(
            transcript_path,
            title=title,
    ...
    print(f"Notes saved: {path} (mode: {mode})")
    return 0
...
def main(argv: list[str] | None = None) -> int:
    ...
    cap.add_argument("--idle", type=float, default=None, help="Auto-stop after N seconds with no new captions")
    cap.add_argument("--output", "-o", default="", help="Transcripts folder override")
    cap.set_defaults(func=_cmd_capture)

    auto_p = sub.add_parser("lecture-auto", help="Capture → parse → RAG notes (unattended)")
    auto_p.add_argument("--idle", type=float, default=None, help="Silence stop in seconds (default from config)")
    auto_p.add_argument("--max-duration", type=float, default=None, help="Max capture seconds")
    auto_p.add_argument("--no-rag", action="store_true", help="Legacy notes without RAG")
    auto_p.add_argument("--fast", action="store_true", help="Apply fast/overnight preset")
    auto_p.set_defaults(func=_cmd_lecture_auto)
